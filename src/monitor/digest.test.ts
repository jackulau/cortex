...
describe("DigestManager", () => {
  ...
  describe("getUndelivered", () => {
    it("queries for undelivered entries with JOINed watch item data", async () => {
  ...
  describe("getByWatchItem", () => {
    it("queries entries for a specific watch item", async () => {
      db._stmt.all.mockResolvedValue({ results: [mockRawEntries[0]] });

      const entries = await manager.getByWatchItem("watch-1");

      expect(entries).toHaveLength(1);
      expect(db._stmt.bind).toHaveBeenCalledWith("watch-1");
      const sql = db.prepare.mock.calls[0][0];
  ...
  describe("markDelivered", () => {
    it("batches update statements for all IDs", async () => {
  ...
  describe("addEntry", () => {
    it("inserts a new digest entry", async () => {
